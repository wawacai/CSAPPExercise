...
float_bits two_float_negate(float_bits f) {
    ...
    return ((sign ^ 0x01) << 31) | (exp << 23) | frac;
}
...
float_bits two_float_absval(float_bits f) {
    ...
    return ((sign ^ 0) << 31) | (exp << 23) | frac;
}

/// 2.94
float_bits two_float_twice(float_bits f) {
    unsigned sign = f >> 31;
    unsigned exp = f >> 23 & 0xff;
    unsigned frac = f & 0x7fffff;
...
/// 2.95
float_bits two_float_half(float_bits f) {
    unsigned sign = f >> 31;
    unsigned exp = f >> 23 & 0xff;
    unsigned frac = f & 0x7fffff;
    ...
    if ((exp ^ 0xff) == 0 && frac != 0) {
        ...
        return (sign << 31) | ((frac >> 1) + (frac & 3) == 3);
    } else if (exp == 1) {
